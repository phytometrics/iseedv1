...
public class PrePostProcessor {
    ...
    static ArrayList<Result> outputsToNMSPredictions(float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY) {
        //ここらへんにnmsのiou閾値の動的変更やconfidense value (result.scoreに相当）の変更をいれてもよいと思う
        ArrayList<Result> results = new ArrayList<>();
        for (int i = 0; i< Configs.mOutputRow; i++) {
            if (outputs[i* Configs.mOutputColumn +4] > Configs.mThreshold) {
                float x = outputs[i* Configs.mOutputColumn];
                float y = outputs[i* Configs.mOutputColumn +1];
                float w = outputs[i* Configs.mOutputColumn +2];
                float h = outputs[i* Configs.mOutputColumn +3];
                ...
                float max = outputs[i* Configs.mOutputColumn +5];
                int cls = 0;
                for (int j = 0; j < Configs.mOutputColumn -5; j++) {
                    if (outputs[i* Configs.mOutputColumn +5+j] > max) {
                        max = outputs[i* Configs.mOutputColumn +5+j];
                        cls = j;
                    }
                }
                ...
                Result result = new Result(cls, outputs[i*Configs.mOutputColumn + 4], rect);
                results.add(result);
            }
        }
        return nonMaxSuppression(results, Configs.mNmsLimit, Configs.mThreshold);
    }
}
