package jp.phytometrics.iseed.tflite;
...
import jp.phytometrics.iseed.MainActivity;
...
//                    options.setAllowBufferHandleOutput(true);
        ...
        if (d.INPUT_SIZE == 640) {
            ...
            //System.out.println("d___________");
        }
        else if (d.INPUT_SIZE == 1280){
            d.output_box = (int) ((Math.pow((inputSize / 64), 2) + Math.pow((inputSize / 32), 2) + Math.pow((inputSize / 16), 2) + Math.pow((inputSize / 8), 2)) * 3);
        }
...
//        d.ANCHORS = anchors;
        ...
        d.numClass = numClass;
        //System.out.println("output classssssssssss______________________");
...
//    private int[] ANCHORS;
    ...
    private static boolean isGPU = false; //ここの処理でgpuをロードするとめちゃんこ遅くなる。なぜだ。。
...
//        int[] intValues = new int[INPUT_SIZE * INPUT_SIZE];
    ...
    public ArrayList<Recognition> _recognizeImage(Bitmap bitmap) {
        //inference. process.
...
//        final ArrayList<Recognition> recognitions = detections;
    ...
    public ArrayList<Recognition> recognizeImage(Bitmap bitmap, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY) {
        //inference. process.
        ByteBuffer byteBuffer_ = convertBitmapToByteBuffer(bitmap);
...
//        float[][][] outbuf = new float[1][output_box][labels.size() + 5];
        ...
        ArrayList<Recognition> detections = new ArrayList<Recognition>();
        ArrayList<Recognition> detections_original = new ArrayList<Recognition>();
        ...
        for (int i = 0; i < output_box; ++i){
            ...
            if (confidenceInClass > getObjThresh()) {
                ...
                float left = imgScaleX * (xPos - w/2);
                float top = imgScaleY * (yPos - h/2);
                float right = imgScaleX * (xPos + w/2);
                float bottom = imgScaleY * (yPos + h/2);


                Log.d("YoloV5Classifier",
                        Float.toString(xPos) + ',' + yPos + ',' + w + ',' + h);

//                final RectF rect_original =
//                        new RectF(
//                                Math.max(0, xPos - w / 2),
//                                Math.max(0, yPos - h / 2),
//                                Math.min(bitmap.getWidth() - 1, xPos + w / 2),
//                                Math.min(bitmap.getHeight() - 1, yPos + h / 2));
                //これはresultiviewにoptimizeされた座標。
